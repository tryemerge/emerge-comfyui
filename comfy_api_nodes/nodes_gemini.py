            ...
            try:
                ...
                if response and response.candidates and len(response.candidates) > 0:
                    ...
                    if attempt == max_attempts:
                        ...
                        # Create detailed error for empty response
                        error_details = create_error_details(
                            ValueError("Gemini returned no candidates in response"),
                            response=response,
                            request_data={"model": model_name, "parts_count": len(parts)}
                        )
                        log_error_details(error_details, "GeminiNode")
                        raise ValueError(create_user_friendly_error_message(error_details))

            except Exception as e:
                last_error = e
                print(f"[EmProps GeminiNode] Error on attempt {attempt}: {str(e)}")
                if attempt == max_attempts:
                ...
                if response and response.candidates and len(response.candidates) > 0:
                    ...
                    if attempt == max_attempts:
                        ...
                        error_details = create_error_details(
                            ValueError("Gemini returned no candidates in response"),
                            response=response,
                            request_data={"model": model_name, "parts_count": len(parts), "aspect_ratio": aspect_ratio}
                        )
                        log_error_details(error_details, "GeminiImageNode")
                        raise ValueError(create_user_friendly_error_message(error_details))

            except Exception as e:
                last_error = e
                print(f"[EmProps GeminiImageNode] Error on attempt {attempt}: {str(e)}")
                if attempt == max_attempts:
                    # Log detailed error information
                    error_details = create_error_details(
                        e,
